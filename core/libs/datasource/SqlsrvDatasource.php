...
class SqlsrvDatasource extends Datasource
{
	...
	public function connection()
	{
		...
		try
		{
			self::$connections[$this->config['connection']] = new PDO('sqlsrv:server='. $this->config['host'] .';Database='. $this->config['name'], $this->config['user'], $this->config['pass']);
			self::$connections[$this->config['connection']]->setAttribute(PDO::ATTR_PERSISTENT, true);
			self::$connections[$this->config['connection']]->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
